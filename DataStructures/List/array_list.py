...
def partition(list, default_sort_criteria, lo, hi):
    ...
    #print(f"Pivote elegido: {elm_pivote}")
    ...
    while i < pivote:
        elm = list["elements"][i]
        #print(f"Comparando {elm} con {elm_pivote}")
        
        if default_sort_criteria(elm, elm_pivote):
            if peque+1 != i:
                #print(f"  -> {elm} es menor, peque es {list["elements"][peque]} intercambio {list['elements'][peque+1]} con {elm}")
        ...
        i += 1 
    if peque + 1 != pivote:  #Si peque + 1 ya es igual a pivote, significa que el pivote ya está en su lugar y no hay que intercambiarlo.
        #print(f"\nIntercambiando pivote {elm_pivote} con {list['elements'][peque+1]}")
